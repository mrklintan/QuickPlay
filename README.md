...
- Continue Play for automatic full-track playback from `00:00` to the natural end.
- Optional DJ Mode with configurable intro skipping, early advancement, natural track-end fallback, and manual-seek protection.
...
- Unified categorized Settings window for Playback, Playlist columns, Keyboard, and Updates.
...
Shortcut assignments can be changed from **Settings → Keyboard**. Audition Start Position, Continue Play, DJ Mode timing, short/long seek durations, **Mark as played after (seconds)**, and the independent **Remove played tracks from playlist** switch are available under **Settings → Settings**. This switch only removes rows from the in-memory playlist; it never deletes audio files. Played time is measured as actual playback time and supports long values such as 600 seconds. If removal is disabled, played tracks remain in the playlist in normal text. If a new shortcut is already in use, QuickPlay asks whether to move it; the previous action then becomes unassigned.

The unified Settings window keeps Playback, Playlist columns, Keyboard, and Updates in one left-navigation layout. Existing Settings menu commands open it directly on the relevant category, while Save applies all changes together and Cancel discards them.

Manual navigation and direct track activation use Audition Start Position. Continue Play automatically starts each next unplayed track at `00:00` and waits for its natural end. DJ Mode is an optional Continue Play mode: **Start next track at** skips the intro and **Advance before track end** controls how early normal forward playback advances. Seeking manually into that final window does not skip the track, and `00:00` waits for natural completion. Natural track-end detection remains the fallback. If no eligible track remains, playback continues with the next sibling folder without wrapping at the final folder. Disabling Continue Play stops playback at the natural end. The current mode can be changed from the compact title-bar selector or the Play menu.

The top-level menus support `Alt+F` for File, `Alt+P` for Play, `Alt+A` for Actions, `Alt+V` for View, `Alt+S` for Settings, and `Alt+B` for About. These access keys do not replace or change configurable direct shortcuts.

## Playlist layout and playback queue

Artist and Title are always the first two playlist columns. Use **Settings → Playlist Columns...** to add, remove, or reorder optional metadata columns, including Disc Number, Track Number, and File Type. File Type uses the TagLib audio format description (for example, `AIFF Audio` or `MPEG Version 1 Audio, Layer 3`) and supports sorting and column filtering. It is blank when the format cannot be read. Drag a column-header divider to resize it, and click a header to toggle ascending or descending sorting. Track Number sorting compares normalized Disc Number first and Track Number second. Values such as `01` and `02/12` are handled safely. Column visibility, order, widths, sort column, and sort direction are saved in the existing QuickPlay settings file.

Uncheck **View → Show filter button** to clear all filters, hide the filter fields and button, and give the playlist its full width. This choice is remembered between launches. While the button is hidden, **Show filters** and **Clear filters** are disabled. Showing the button again enables **Show filters**; **Clear filters** is enabled only when a filter is active. Use **View → Show filters** or the compact funnel icon at the right of the column headings to show or collapse the filter fields; QuickPlay remembers this choice. Collapsing keeps filters active, and the button shows the number of active column filters. **View → Clear filters** removes every filter, even while the fields are collapsed. Type in the filter box beneath any playlist column heading to show rows containing that text, ignoring letter case. For example, `dance` matches `dance`, `Dance`, and `eurodance`. Multiple column filters must all match. Clear a box (or press Escape in it) to remove that filter. Sorting remains available by clicking the column name. Filters only affect visible rows; playback navigation and the saved playlist retain the full queue. Filter button visibility, filter field visibility, and all column filter texts are saved in settings.json and restored at startup. Hiding a column clears its filter. Hiding the filter button also clears saved filters.
